...
#%%
df = pd.read_csv('dados/eventos_prioridade_maurilio.csv', sep='|')
...
#%%
dias = [x for x in list(set(df['data'])) if str(x) != 'nan']
print(dias)
...
for dia in dias:
    ...
    # eventos da manha
    ev = df.loc[(df['data'] == dia) & \
                (df['inicio'] >= pd.to_datetime('08:00:00').time()) & \
    ...
    ev = df.loc[(df['data'] == dia) & \
                ...
                (df['inicio'] < pd.to_datetime('18:00:00').time())]
...
df_combos['custo'] = df_combos.apply(lambda x : max_min(x['prioridade'], min_prioridade, max_prioridade) + 
                                                ...
                                                max_min(x['distancia'], min_distancia, max_distancia) / 2.0, axis=1)

#%%
df_combos.sort_values(by=['dia', 'turno', 'custo'], inplace=True)
df_combos.to_csv('dados/combos_maurilio.csv', sep='|', index=False)

#%%
selecao = df_combos.groupby(by=['dia', 'turno']).first()

#%%
df.loc[df['id'].isin(selecao['evento_1'].tolist() + selecao['evento_2'].tolist())].to_csv('dados/selecao_maurilio.csv', sep='|', index=False)
